...
def absvit_base_patch16_224(pretrained=False, **kwargs):
    model = VisionTransformer(
        patch_size=16, embed_dim=768, num_classes=11, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        # checkpoint = torch.hub.load_state_dict_from_url(
        ...
        #     map_location="cpu", check_hash=True
        # )
        checkpoint = torch.load('/home/zhengjl/mydata/SSFNet/absvit_base_patch16_224.pth')
