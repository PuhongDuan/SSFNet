...
def mobile_vit_small(pretrained=False, num_classes: int = 1000):
    ...
    m = MobileViT(config, num_classes=num_classes)
    if pretrained:
        # m = load_mobilevit_weights("./checkpoints/model_best.pth.tar")
        m.load_state_dict(torch.load('./checkpoints/mobilevit_s.pt'), strict=False)
    return m

def load_mobilevit_weights(model_path):
  # Create an instance of the MobileViT model
  # net = mobile_vit_small(pretrained=False)
  config = get_config("small")
  net = MobileViT(config, num_classes=11)
