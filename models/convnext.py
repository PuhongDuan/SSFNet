...
class ConvNeXt(nn.Module):
    ...
    def __init__(self, model_name: str = 'T') -> None:
        ...
        for i in range(4):
            stage = nn.Sequential(*[Block(self.channels[i], dpr[cur+j]) for j in range(self.depths[i])])
...
def convnext(pretrained=True):
    model = ConvNeXt('T')
    if pretrained:
        model.load_state_dict(torch.load('./checkpoints/convnext_tiny_1k_224_ema.pth'), strict=False)
    return model
